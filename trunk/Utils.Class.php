...
class DB_Mysql{
   ...
   * @return resource $src
   */
  function newConnect($base, $host, $user, $pass, $codepage = ''){
    $src = mysql_pconnect($host, $user, $pass);
    // выбираем базу
  ...
  function myError($src, $sql, $stop){
    if($stop == true and is_resource($src))
    {
      ...
      $sql
      ----------------------------------------
      mysql_errno: ".mysql_errno( $src )."
      mysql_error: ".mysql_error( $src )."
      ========================================
      </xmp>
      ";
      die( $text );
...
class utils{
  ...
  function inverter($value)
  {
    ...
    return strtr($value, array_merge($hash, array_flip( $hash )));
  }

  // вырезание всех спецсимволов за исключением русских и англицких букв и цифр.
  // Пробелы заменяются на _
  function tagger($word, $replace_by = '_', $strip = true)
  {
    $qt = preg_quote($replace_by, '-');
    $word = preg_replace("|[^a-zа-я0-9$qt]|i", "$replace_by", strtr(trim($word,'. '), ' ',"$replace_by" ));
    if($replace_by <> '' && $strip == true ) $word = preg_replace("/[$qt]{2,}/", "$replace_by", $word);
    return strtolower(trim($word,"$replace_by"));
  }
  ...
  function  razbor($str)
  {
    return trim(ereg_replace("[[:space:]]+"," ", implode(" ", explode("\n",trim($str, "\r\t\n")))));
  }
  ...
  */
  function prr($array, $die = true, $console = false)
  {
     print "\n".(($console == false)? "<xmp>\n":'');
     print_r($array);
     print "\n".(($console == false)? "</xnp>\n":'');
